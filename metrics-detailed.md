...
| Метрика | MFE | Обоснование |
|---------|-----|-------------|
| `redirectMs` | | Серверная конфигурация, 0 у обоих |
| `dnsMs` | | DNS lookup основного документа — сетевая инфраструктура, не зависит от архитектуры фронтенда |
| `connectMs` | | TCP handshake основного документа — одинаков при одном сервере |
| `sslMs` | | TLS handshake основного документа — одинаков при одном сервере |
| `requestMs` | | Время ожидания ответа сервера — серверная обработка, оба отдают статический HTML через nginx |
| `responseMs` | **+** | Размер HTML документа различается: MFE Shell — минимальный HTML-каркас, монолит — больше script/link тегов |
...
| `subResourcesMs` | **+** | MFE подгружает ресурсы по мере навигации (lazy), монолит загружает больше при старте |
...
| `dnsCached` | | Флаг кэширования DNS — инфраструктурный параметр, не зависит от архитектуры |
| `connectionReused` | | Флаг переиспользования TCP — инфраструктурный параметр |
| `redirectHidden` | | Флаг cross-origin редиректа, 0 у обоих |

**Итог:** 7 из 15 метрик релевантны. Сетевые фазы (DNS, TCP, TLS, request) одинаковы — оба варианта обслуживаются одним сервером. Различия начинаются с фазы получения HTML: размер документа, парсинг DOM, выполнение скриптов и загрузка подресурсов.
...
| `speedIndexScore` | | Оценка Speed Index (0–100) — дублирует speedIndex в другом формате, не добавляет информации |

**Итог:** 7 из 8 метрик релевантны. Lighthouse — главный инструмент сравнения.

### 5. FPS Metrics (page.fps)

| Метрика | MFE | Обоснование |
|---------|-----|-------------|
| `avgFps` | | Замеряется на idle-странице после загрузки — ~60fps у обоих вариантов |
| `minFps` | | Аналогично — нет значимой разницы на idle |
| `maxFps` | | Лучший кадр — обычно одинаков (ограничен VSync) |
| `droppedFrames` | | Не фиксируются на idle-странице |
| `droppedFramesPercent` | | Аналогично |
| `totalFrames` | | Зависит от длительности замера, не от архитектуры |
| `durationMs` | | Параметр замера, не метрика производительности |

**Итог:** 0 из 7 метрик релевантны. FPS замеряется на idle-странице после загрузки — оба варианта показывают ~60fps. Различия в плавности лучше фиксируются через TBT, Long Tasks и FPS в составе E2E-сценария.
...
| `totalApiDurationMs` | | Зависит от бэкенда |
...
| Метрика | MFE | Обоснование |
|---------|-----|-------------|
| `unusedJsPercent` | | В эксперименте не показал значимой разницы |
| `unusedCssPercent` | | В эксперименте не показал значимой разницы |
| `jsTotalBytes` | | Аналогично |
| `jsUnusedBytes` | | Аналогично |
| `cssTotalBytes` | | Аналогично |
| `cssUnusedBytes` | | Аналогично |
...
| Метрика | MFE | Обоснование |
|---------|-----|-------------|
| `avgContentDownloadMs` | | В эксперименте не показал значимой разницы |
| `maxContentDownloadMs` | | Аналогично |
| `http2Percent` | | Оба варианта используют HTTP/2, значение одинаковое |
| `http3Percent` | | HTTP/3 не используется ни в одном варианте |

#### 6.7 Cache / Service Worker

| Метрика | MFE | Обоснование |
|---------|-----|-------------|
| `cacheHitPercent` | | Замеры в incognito — кэш не используется, метрика не информативна |
| `cacheHitCount` | | Аналогично |
...
| Метрика | MFE | Обоснование |
|---------|-----|-------------|
| `totalDurationMs` | | Включает время загрузки страницы — дублирует Navigation Timing |
| `scenarioDurationMs` | **+** | Время выполнения шагов без загрузки |
| `totalLongTasks` | | В эксперименте 0 у обоих вариантов |
...
| `avgFps` (E2E) | | В эксперименте не показал значимой разницы |
| `minFps` (E2E) | | Аналогично |
| `totalFrames` (E2E) | | Зависит от длительности замера, не от архитектуры |
| `droppedFrames` (E2E) | | В эксперименте не показал значимой разницы |
...
| `totalBrotli` | | В эксперименте не использовался (сервер отдаёт gzip) |
...
| `duplicatedSize` / `duplicatedGzip` | | В эксперименте не выделялся отдельно (учтён в runtimeSize) |
| `{type}.size/gzip` | **+** | Разбивка по типу файлов |
| `largestChunkGzip` | | В эксперименте не измерялся отдельно от largestChunkSize |
...
| 4 | Lighthouse | 8 | 7 | 88% |
| 5 | FPS | 7 | 0 | 0% |
| 6 | Resource Timing + V8 | 43 | 25 | 58% |
| 7 | E2E Metrics | 12 | 6 | 50% |
| 8 | Bundle Analysis | 10 | 7 | 70% |
| | **Итого** | **105** | **52** | **50%** |
